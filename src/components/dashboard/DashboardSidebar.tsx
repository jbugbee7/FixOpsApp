...
const DashboardSidebar = ({ activeTab, onTabChange }: DashboardSidebarProps) => {
  ...
  const displayMenuItems = isMobile 
    ? [
        ...menuItems.filter(item => ['dashboard', 'work-order'].includes(item.value)),
        ...mobileOnlyItems, // AI Assistant
        ...menuItems.filter(item => ['settings', 'logout'].includes(item.value))
      ]
    : menuItems;
    ...
    <Sidebar side="right" className="bg-card/95 dark:bg-background/95 backdrop-blur-xl border-l border-border/50 z-50">
      <SidebarHeader className="bg-card/90 dark:bg-card/50 backdrop-blur-xl">
        <div className="flex items-center gap-3 px-4 py-1.5">
          ...
          <div className="text-sm font-bold text-foreground mt-0 md:mt-2">
            FixOps
          </div>
        </div>
      </SidebarHeader>
